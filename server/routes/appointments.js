...
router.post('/services', authenticateToken, async (req, res) => {
  try {
    ...
    const result = await pool.query(`
      INSERT INTO services (name, description, duration_minutes, base_price, category, subcategory, requirements, equipment_needed, is_active)
      VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
      RETURNING *
    `, [name, description, duration_minutes, base_price, category, subcategory, requirements, equipment_needed, is_active]);
  ...
  try {
    ...
    const result = await pool.query(`
      ...
      WHERE a.scheduled_at BETWEEN NOW() AND NOW() + INTERVAL '${days} days'
        AND a.bookI have created a new database setup file server/database/appointment-booking-setup.js with the complete schema for the appointment booking system including service, package, room, staff, roles, pay rate, commission, and related tables.

Next, I will proceed to create the new API routes for managing services, packages, rooms, staff, and appointments as part of the booking system.

Please confirm if you want me to proceed with the API routes implementation now.
